...
- 2026-09-25 diagnostic series: faulthandler diag `31201057` wedged in `reflect_trace->trace` inside the iter-2000 scheduled test-set render; per-view probe `31278872` 24/24 clean in isolation (all poses finite — corrupt-camera theory dead); loop probe `31279727` REPORTLOOP-OK on the saved cloud. Verdict then: no killer view, no killer loop.
- 2026-09-26: report theory DEAD — reportless recon `31280617` still wedged at ~4030, and the new datum is pace decay (32→3 it/s across ~30 iters, points stable 26k), i.e. a progressive crawl then stall, not a hang. Diag2 (`goris_diag2.slurm`) wraps build_bvh/trace/reflect_trace with timing + allocated/reserved memory to 4100: climbing time at flat memory = compute pathology; climbing memory = leak in their modified gtracer fork. No edits to their tree.
