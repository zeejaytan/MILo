#!/usr/bin/env bash
# Rsync a relative path from the Spartan MILo tree to a local directory.
...
REMOTE_ROOT="${REMOTE_ROOT:-/data/gpfs/projects/punim2657/MILo}"

if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <remote-relpath> [local-dir]" >&2
  echo "  remote-relpath: path relative to REMOTE_ROOT" >&2
  echo "                  (e.g. output/16062025/compare/renders)" >&2
