# MILo traps — the detail behind the index in `AGENTS.md`

Moved from `AGENTS.md` on 2026-09-11 so it loads only when needed; text unchanged except
cross-references. Withdrawn claims are kept struck through on purpose.

## DTU route: getting past the block cliff (addendum, 2026-09-02)
...
- **What actually limits TSDF resolution here — measured on A03, not assumed.** Three
  ...
  3. *Truncation band*, `trunc_voxel_multiplier × voxel_size` = **±6.0 mm** at the
     ...
     — see fork change 4 in `fork-changes.md`.
...
- **Open3D's CPU/CUDA choice is made at import, on whatever node you are sitting on — do
  ...
  **An Open3D CUDA OOM is not a recoverable exception**: it raises, then aborts the
  ...
  overridable with `--o3d_device` (see fork change 4 in `fork-changes.md`). Fusion cost grows as 1/voxel², so
