...
- 2026-09-19: smoke `30721180` FAILED 1:0 on `mediapy` (pure-python gap in render_utils); sweep found `lpips` waiting at the next stage too; both added `--no-deps` (wandb/tensorboard exempt: try/except-guarded, `--logger none`). Resubmit `30752840` COMPLETED 0:0 — full import cover green. Ticket closed.
